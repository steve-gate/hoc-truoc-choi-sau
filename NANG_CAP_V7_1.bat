...
setlocal
cd /d "%~dp0"
title FocusLock V7.1 - Nang cap Strict Blocking
...
echo.
echo ==========================================
echo FOCUSLOCK V7.1 - STRICT BLOCKING
echo ==========================================
echo.
...
echo.
if not "%RC%"=="0" (
  echo [LOI] Nang cap V7.1 that bai. Ma loi: %RC%
  if exist "%~dp0install.log" echo Log: %~dp0install.log
  pause
  exit /b %RC%
)

echo [OK] FocusLock V7.1 da build + cai dat xong.
echo [NHAC] Reload Browser Extension mot lan.
echo.
pause
exit /b 0
