...
echo.
if not "%RC%"=="0" (
  echo [LOI] Nang cap V7 that bai. Ma loi: %RC%
  if exist "%~dp0install.log" echo Log: %~dp0install.log
  pause
  exit /b %RC%
)

echo [OK] FocusLock V7 da build + cai dat xong.
echo [NHAC] Reload Browser Extension va refresh tab hoc dang mo.
echo.
pause
exit /b 0
