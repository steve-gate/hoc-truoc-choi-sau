...
if (-not (Test-Admin)) { throw "Mo CMD bang Run as administrator, sau do chay NANG_CAP_V7_6_0.bat." }
...
$stageRoot = Join-Path $root ".build-v760"
$stageApp = Join-Path $stageRoot "App"
$stageService = Join-Path $stageRoot "Service"
$stageNative = Join-Path $stageRoot "NativeHost"

Write-Host ""
Write-Host "==> FocusLock V7.6.0 - build App + Service + NativeHost" -ForegroundColor Cyan
...
$slotName = "NativeHost-" + (Get-Date -Format "yyyyMMdd-HHmmss") + "-v760"
...
if($ver -notlike "7.6.0*"){throw "Wrong App version: $ver"}
...
if ($publishedExtensionVersion -ne "7.6.0") { throw "Wrong published extension version: $publishedExtensionVersion" }

Start-Process $targetAppExe

Write-Host ""
Write-Host "============================================================" -ForegroundColor Green
Write-Host "OK - FOCUSLOCK V7.6.0 INSTALLED" -ForegroundColor Green
