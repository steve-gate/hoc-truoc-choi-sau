...
if (-not (Test-Admin)) { throw "Mo CMD bang Run as administrator, sau do chay NANG_CAP_V7_6_1.bat." }
...
$stageRoot = Join-Path $root ".build-v761"
$stageApp = Join-Path $stageRoot "App"
$stageService = Join-Path $stageRoot "Service"
$stageNative = Join-Path $stageRoot "NativeHost"

Write-Host ""
Write-Host "==> FocusLock V7.6.1 - build App + Service + NativeHost" -ForegroundColor Cyan
...
$slotName = "NativeHost-" + (Get-Date -Format "yyyyMMdd-HHmmss") + "-v761"
...
if (!(Test-Path $hostExe)) { throw "NativeHost V7.6 missing." }

$manifestPath = Join-Path $slot "com.focuslock.browserbridge.json"
$manifest = @{
  name = $hostName
  description = "FocusLock Browser Bridge V7.6"
  path = $hostExe
  type = "stdio"
...
if($ver -notlike "7.6.1*"){throw "Wrong App version: $ver"}
...
if ($publishedExtensionVersion -ne "7.6.1") { throw "Wrong published extension version: $publishedExtensionVersion" }

Start-Process $targetAppExe

Write-Host ""
Write-Host "============================================================" -ForegroundColor Green
Write-Host "OK - FOCUSLOCK V7.6.1 INSTALLED" -ForegroundColor Green
Write-Host "Web timing no longer requires desktop-agent heartbeat." -ForegroundColor Green
